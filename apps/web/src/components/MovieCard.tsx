    ...
    <article className="group relative flex bg-surface-dark rounded-2xl overflow-hidden shadow-lg border border-white/5 hover:border-primary/30 transition-[transform,box-shadow,border-color] duration-300 hover:shadow-primary/5 hover:-translate-y-1 h-48">
      ...
      <div className="flex-1 p-4 flex flex-col justify-center">
        ...
        <div className="flex items-center gap-3 text-sm text-slate-400 mb-2">
          ...
          <span className="text-xs border border-slate-600 px-1.5 rounded text-slate-400">{movie.duration}</span>
        </div>
        
        <p className="text-xs text-slate-500 line-clamp-2 mb-2">{movie.plot}</p>

        <div className="flex content-center gap-1.5 mb-2">
            <Star className="text-amber-gold h-5 fill-current" />
            <span className="text-amber-gold font-bold text-base">{movie.rate}</span>

        </div>

        <div className="flex flex-wrap gap-2 mt-auto">
          {movie.genre.slice(0, 3).map((g) => (
            <span key={g} className="px-2.5 py-0.5 rounded-full bg-primary/10 text-primary-light text-[10px] font-medium border border-primary/20 whitespace-nowrap">
              {g}
            </span>
          ))}
        </div>
